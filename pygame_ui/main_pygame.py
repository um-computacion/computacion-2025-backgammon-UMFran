...
def main():
    ...
    while running:
            ...
            # --- Eventos Juego ---
                ...
                if event.type == pygame.MOUSEBUTTONDOWN:
                    ...
                    if boton_clicado == "ROLL_DICE":
                        ...
                        # Obtener la "barra" (fichas comidas) desde el Board
                        banco_actual_temp = game.__board__.get_barra(color_actual_temp)
        ...
        # --- Dibujado ---
        # Usar el color de fondo principal definido en las constantes
        pantalla.fill(C.COLOR_FONDO_PRINCIPAL)
        if game_state == "MENU":
            interfaz.dibujar_menu(nombre_p1, nombre_p2, input_activo)
        else:
            interfaz.dibujar_tablero()
            # Dibujar fichas: Tablero + bancos (barra). Las "home" se usan como contadores
            banco_white = game.__board__.get_barra("white")
            banco_black = game.__board__.get_barra("black")
            home_white = game.__board__.get_afuera("white")
            home_black = game.__board__.get_afuera("black")

            interfaz.dibujar_fichas(
                game.mostrar_tablero(), banco_white, banco_black
            )
            # Pasar longitudes/contadores a la UI (firma: turno_color, dados, game_state, mensaje_error,
            # origen_seleccionado, posibles_destinos, banco_blanco_len, banco_negro_len, home_blanco_len, home_negro_len)
            interfaz.dibujar_ui(
                ...
                len(banco_white), len(banco_black), len(home_white), len(home_black)
            )
