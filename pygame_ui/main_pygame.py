    ...
    while running:
            ...
            # --- Eventos Juego ---
                ...
                if event.type == pygame.MOUSEBUTTONDOWN:
                    ...
                    if boton_clicado == "ROLL_DICE":
                        ...
                        if origen_temporal:
                            ...
                            for i in range(24):
                                ...
                                if casilla_temp and casilla_temp[0] == color_actual_temp:
                                    ...
                                    in_home_board = (color_actual_temp=="white" and i>=18) or (color_actual_temp=="black" and i<=5)
                                    if in_home_board:
                        ...
                        if hay_movimiento: game_state = "MAKE_MOVE"
                        else: mensaje_error = "No hay movimientos. Pasa el turno."; game_state = "MAKE_MOVE"
                    ...
                    if origen_seleccionado == 'bar':
                        ...
                        else: mensaje_error = "Movimiento inválido"; origen_seleccionado = None; posibles_destinos = []
                        ...
                        origen_potencial = None; puede_sacar_directo = False
                        if casilla_clicada == 'bar' and len(banco_actual) > 0: origen_potencial = 'bar'
                            ...
                            if casilla_origen and casilla_origen[0] == color_actual:
                                if len(banco_actual) > 0: mensaje_error = "Debes reingresar primero"
                                else:
                                    try:
                                        in_home_board = (color_actual == "white" and casilla_clicada >= 18) or (color_actual == "black" and casilla_clicada <= 5)
                                        if in_home_board:
                                            todas_en_cuadrante = True; rango_check_fuera = range(18) if color_actual == "white" else range(6, 24)
                                            for i in rango_check_fuera:
                                                if color_actual in game.mostrar_tablero()[i]: todas_en_cuadrante = False; break
                                            if todas_en_cuadrante and game_state == "MAKE_MOVE":
                                                mov_exacto = (24 - casilla_clicada) if color_actual == "white" else (casilla_clicada + 1)
                                                dado_valido = None; dados_sim = game.__dados__.get_dados()
                                                if mov_exacto in dados_sim: dado_valido = mov_exacto
                                                else:
                                                    dados_mayores = sorted([d for d in dados_sim if d > mov_exacto])
                                                    if dados_mayores:
                                                         es_mas_lejana = True; rango_check_lejana = range(18, casilla_clicada) if color_actual == "white" else range(casilla_clicada + 1, 6)
                                                         for i in rango_check_lejana:
                                                             if color_actual in game.mostrar_tablero()[i]: es_mas_lejana = False; break
                                                         if es_mas_lejana: dado_valido = dados_mayores[0]
                                                if dado_valido is not None:
                                                    game.sacar(casilla_clicada); dados_disponibles = game.__dados__.get_dados()
                                                    origen_seleccionado = None; posibles_destinos = []; puede_sacar_directo = True
                                                else: origen_potencial = casilla_clicada
                                            else: origen_potencial = casilla_clicada
                                        else: origen_potencial = casilla_clicada
                                    except Exception: origen_potencial = casilla_clicada

                        if origen_potencial is not None and not puede_sacar_directo:
                             if game_state == "MAKE_MOVE":
                                origen_seleccionado = origen_potencial; posibles_destinos = []
                                ...
                                if isinstance(origen_seleccionado, int):
                                     try:
                                         in_home = (color_actual=="white" and origen_seleccionado>=18) or (color_actual=="black" and origen_seleccionado<=5)
                                         if in_home:
                                             todas_home = True; r_fuera = range(18) if color_actual=="white" else range(6,24)
                                             for i in r_fuera:
                                                 if color_actual in game.mostrar_tablero()[i]: todas_home=False; break
                                             if todas_home:
                                                 mov_s = (24-origen_seleccionado) if color_actual=="white" else (origen_seleccionado+1)
                                                 d_sim = game.__dados__.get_dados()
                                                 if mov_s in d_sim: puede_sacar_desde_aqui=True
                                                 else:
                                                     d_may = sorted([d for d in d_sim if d > mov_s])
                                                     if d_may:
                                                         es_lej = True; r_lej = range(18, origen_seleccionado) if color_actual=="white" else range(origen_seleccionado+1, 6)
                                                         for i in r_lej:
                                                             if color_actual in game.mostrar_tablero()[i]: es_lej=False; break
                                                         if es_lej: puede_sacar_desde_aqui=True
                                     except: pass
                                if not posibles_destinos and not puede_sacar_desde_aqui:
                                     mensaje_error = "No hay movimientos válidos"; origen_seleccionado = None
                             else: mensaje_error = "Debes tirar los dados"
                        elif not puede_sacar_directo: origen_seleccionado = None; posibles_destinos = []
        ...
        if game and game_state != "GAME_OVER":
            if game.juego_terminado():
                ...
                if ganador: game_state = "GAME_OVER"

        # --- Dibujado ---
        # --- CORRECCIÓN: Usar siempre COLOR_FONDO_PRINCIPAL ---
        ...
        if game_state == "MENU":
            ...
            interfaz.dibujar_fichas(
                game.mostrar_tablero(), banco_w, banco_b, home_w, home_b
            )
            interfaz.dibujar_ui(
