...
# Importar la clase de dados para simular "no hay movimientos"
...
class TestCLI(unittest.TestCase):
    ...
    def test_mostrar_juego_basico(self, mock_print, mock_randint):
        ...
        self.assertIn(
            ("Turno de: Fran",), [args[0] for args in llamadas if args]
        )
        self.assertIn(
            ("==========================================================",),
    ...
    def test_mostrar_banco_con_fichas_blancas_y_negras(self, mock_print):
        ...
        self.juego.__board__.__barra__["white"].extend(["white", "white"])
        self.juego.__board__.__barra__["black"].extend(["black", "black", "black"])
    ...
    def test_mostrar_banco_solo_fichas_blancas(self, mock_print):
        self.interfaz = cli("Fran", "Maria")
        self.juego = self.interfaz.__game__
        self.juego.__board__.__barra__["white"].extend(["white", "white", "white"])
        self.interfaz.mostrar_banco()
        mock_print.assert_any_call("\nFICHAS CAPTURADAS:")
    ...
    def test_mostrar_banco_solo_fichas_negras(self, mock_print):
        self.interfaz = cli("Fran", "Maria")
        self.juego = self.interfaz.__game__
        self.juego.__board__.__barra__["black"].extend(["black"])
        self.interfaz.mostrar_banco()
        mock_print.assert_any_call("\nFICHAS CAPTURADAS:")
    ...
    def test_mostrar_banco_vacio(self, mock_print):
        self.interfaz = cli("Fran", "Maria")
        self.juego = self.interfaz.__game__
        self.juego.__board__.__barra__["white"].clear()
        self.juego.__board__.__barra__["black"].clear()
        self.interfaz.mostrar_banco()
    ...
    def test_mostrar_banco_muchas_fichas(self, mock_print):
        self.interfaz = cli("Fran", "Maria")
        self.juego = self.interfaz.__game__
        self.juego.__board__.__barra__["white"].extend(["white"] * 10)
        self.juego.__board__.__barra__["black"].extend(["black"] * 15)
        self.interfaz.mostrar_banco()
        mock_print.assert_any_call("\nFICHAS CAPTURADAS:")
    ...
    def test_mostrar_home_con_fichas_blancas_y_negras(self, mock_print):
        self.interfaz = cli("Fran", "Maria")
        self.juego = self.interfaz.__game__
        self.juego.__board__.__afuera__["white"].extend(["white", "white", "white"])
        self.juego.__board__.__afuera__["black"].extend(["black", "black"])
    ...
    def test_mostrar_home_solo_fichas_blancas(self, mock_print):
        self.interfaz = cli("Fran", "Maria")
        self.juego = self.interfaz.__game__
        self.juego.__board__.__afuera__["white"].extend(["white"] * 4)
        self.interfaz.mostrar_home()
        mock_print.assert_any_call("\nFICHAS EN CASA:")
    ...
    def test_mostrar_home_solo_fichas_negras(self, mock_print):
        self.interfaz = cli("Fran", "Maria")
        self.juego = self.interfaz.__game__
        self.juego.__board__.__afuera__["black"].extend(["black"] * 5)
        self.interfaz.mostrar_home()
        mock_print.assert_any_call("\nFICHAS EN CASA:")
        mock_print.assert_any_call("  Blancas: 0")
    ...
    def test_mostrar_home_vacio(self, mock_print):
        self.interfaz = cli("Fran", "Maria")
        self.juego = self.interfaz.__game__
        self.juego.__board__.__afuera__["white"].clear()
        self.juego.__board__.__afuera__["black"].clear()
        self.interfaz.mostrar_home()
        mock_print.assert_any_call("\nFICHAS EN CASA:")
        mock_print.assert_any_call("  Blancas: 0")
        mock_print.assert_any_call("  Negras: 0")
    ...
    def test_mostrar_home_todas_fichas_sacadas(self, mock_print):
        self.interfaz = cli("Fran", "Maria")
        self.juego = self.interfaz.__game__
        self.juego.__board__.__afuera__["white"].extend(["white"] * 15)
        self.juego.__board__.__afuera__["black"].extend(["black"] * 8)
        self.interfaz.mostrar_home()
        mock_print.assert_any_call("\nFICHAS EN CASA:")
    ...
    def test_movimientos_posibles_fichas_blancas_con_dados(self, mock_print):
        ...
        mock_print.assert_any_call("  6 → 8 (usando dado 2)")
        mock_print.assert_any_call("  6 → 9 (usando dado 3)")
    ...
    def test_movimientos_posibles_fichas_blancas_multiples_posiciones(
        ...
        self.interfaz.mostrar_movimientos_posibles()
        mock_print.assert_any_call("\nMovimientos posibles para white:")
        mock_print.assert_any_call("  1 → 2 (usando dado 1)")
        mock_print.assert_any_call("  11 → 12 (usando dado 1)")
    ...
    def test_movimientos_posibles_fichas_negras_con_dados(self, mock_print):
        ...
        mock_print.assert_any_call("  21 → 20 (usando dado 1)")
        mock_print.assert_any_call("  21 → 17 (usando dado 4)")
    ...
    def test_movimientos_posibles_fichas_negras_multiples_posiciones(
        ...
        self.interfaz.mostrar_movimientos_posibles()
        mock_print.assert_any_call("\nMovimientos posibles para black:")
        mock_print.assert_any_call("  16 → 14 (usando dado 2)")
        mock_print.assert_any_call("  24 → 22 (usando dado 2)")
    ...
    def test_movimientos_posibles_fichas_blancas_fuera_limite(self, mock_print):
        ...
        self.assertFalse(any("23 → 28" in s for s in call_strings))
    ...
    def test_movimientos_posibles_fichas_negras_fuera_limite(self, mock_print):
        ...
        self.interfaz.mostrar_movimientos_posibles()
        call_strings = [str(call[0]) for call in mock_print.call_args_list]
        self.assertFalse(any("2 → -1" in s for s in call_strings))
    ...
    def test_mostrar_estado_llamadas_reales(self, mock_print):
        ...
        self.juego.__board__.__barra__["white"].append("white")
        self.interfaz.mostrar_estado_completo()
        mock_print.assert_any_call("="*50)
    ...
    def test_mostrar_estado_con_fichas_en_home_y_banco(self, mock_print):
        self.interfaz = cli("Fran", "Maria")
        self.juego = self.interfaz.__game__
        self.juego.__board__.__barra__["black"].append("black")
        self.juego.__board__.__afuera__["white"].append("white")
        self.interfaz.mostrar_estado_completo()
        mock_print.assert_any_call("="*50)
    ...
    def test_menu_mostrar_ganador_jugador1(self, mock_print, mock_input):
        ...
        interfaz.__game__.__juego_terminado__ = True
    ...
    # --- INICIO CORRECCIÓN ASSERT ---
    @patch("builtins.input", side_effect=["1", "1", "4", "4"])
    @patch("builtins.print")
    def test_mover_ficha_dado_invalido(self, mock_print, mock_input):
        ...
        # Verificar que el print de error (que es un solo string) fue llamado
        llamadas_print = [str(call[0][0]) for call in mock_print.call_args_list if call[0]]
        self.assertTrue(any(s.startswith("Error:") for s in llamadas_print))
    # --- FIN CORRECCIÓN ASSERT ---
    ...
    def test_reingresar_ficha_valido(self, mock_reingresar, mock_input):
        interfaz = cli("Fran", "Maria")
        juego = interfaz.__game__
        juego.__board__.__barra__["white"].append("white")
        juego.__dados__.__movimientos__ = [2]
        interfaz.jugar_turno()
    ...
    @patch("builtins.input", side_effect=["3", "24", "4"])
    ...
    def setup_sacar_valido(self, juego, color, origen_idx):
        """Helper para tests de sacar."""
        for i in range(24):
            juego.__board__.__contenedor__[i].clear()
        if color == "white":
            juego.__board__.__contenedor__[origen_idx] = ["white"]
        else:
            juego.__turno__ = juego.__jugador2__
            juego.__board__.__contenedor__[origen_idx] = ["black"]

    # --- INICIO CORRECCIÓN (StopIteration y Assert) ---
    @patch("builtins.input", side_effect=["3", "4"]) # Opción 3, luego Salir
    @patch("builtins.print")
    def test_sacar_ficha_fuera_cuadrante(self, mock_print, mock_input):
        interfaz = cli("Fran", "Maria")
        juego = interfaz.__game__
        juego.__dados__.__movimientos__ = [1]
        # Usar tablero inicial (white tiene fichas en 0, 11, etc.)
        interfaz.jugar_turno()
        # Debe imprimir error y NO pedir 'Origen'
        self.assertTrue(
            any("fuera de tu cuadrante final" in str(call[0][0])
